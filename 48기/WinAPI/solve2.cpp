...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	HDC					hdc;
	char				szBuf[128] = "가오나시";
	static int			nPosX = 400;
	static int			nPosY = 400;
	static const int	nMoveSpeed = 10;

	switch(iMessage)
	{
		case WM_PAINT:
			hdc = BeginPaint(hWnd, &ps);
			::TextOut(hdc, nPosX, nPosY, szBuf ,strlen(szBuf));
			EndPaint(hWnd, &ps);
		break;
		case WM_LBUTTONDOWN:
		...
		case WM_KEYDOWN:
			...
			switch(wParam)
			{
				...
				break;
				case VK_LEFT:
					nPosX = max(rc.left, nPosX - nMoveSpeed);
				break;
				case VK_UP:
					nPosY = max(rc.top, nPosY - nMoveSpeed);
				break;
				case VK_RIGHT:
					nPosX = min(rc.right, nPosX + nMoveSpeed);
				break;
				case VK_DOWN:
					nPosY = min(rc.bottom, nPosY + nMoveSpeed);
				break;
			}
